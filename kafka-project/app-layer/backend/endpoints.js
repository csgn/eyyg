...
router.get("/methods", async (req, res) => {
  pool.query(
    `SELECT DISTINCT request_method FROM serverLog where request_endpoint='${req.query.url}'`,
    (error, results) => {
      if (error) {
        throw error;
      }
...
router.get("/statuses", async (req, res) => {
  pool.query(
    `SELECT DISTINCT request_status FROM serverLog where request_endpoint=${req.query.url}`,
    (error, results) => {
      if (error) {
        throw error;
      }
